...
namespace Meg.Core.Exercises
{
   public abstract class ExercisesExamples
   {
      ...
      protected ExercisesExamples(IExpressionFormatVisitor visitor) => this.visitor = visitor;
      ...
      protected IEnumerable<Exercise> GetSumOfDivWith2ConstAndNumber(string label, int problemCount)
      {
         Exercise GetExercise(int id)
         {
            ...
            var numbers = Enumerable.Range(0, 2).Select(_ => GetRandomNumber(2, 10)).ToList();
            var computedResult = numbers.Aggregate((a, b) => a * b);
            var c = Constant.New(GetRandomNumber(10, 50));
            ...
            var div = Division.NewInline(visitor, constants);
            var sum = Sum.New(visitor, div, c);
            var res = Constant.NewAsUnknown(sum.Compute(), Unknowns.Dots);
            var eq = Equality.New(sum, res);
      ...
      protected IEnumerable<Exercise> GetSumWith4ConstantsMixedSignes(string label, int problemCount)
      {
         ...
         Exercise GetExercise(int id)
         {
            ...
            var sum_ab = Sum.New(visitor, a, b);
            var sub_ab_c = Subtraction.New(visitor, sum_ab, c);
            var sum_abc_d = Sum.New(visitor, sub_ab_c, d);
            var res = Constant.NewAsUnknown(sum_abc_d.Compute(), Unknowns.Dots);
            var eq = Equality.New(sum_abc_d, res);
